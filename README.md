...
心理陪伴助手 MVP，用于展示“本地/在线大模型接入 + Prompt Engineering + RAG + 记忆 + 规则风险识别 + LLM 结构化分类 + 工具调用”的 AI Agent 工作流搭建能力。系统定位是心理陪伴、心理知识问答、咨询分流和高风险预警，不提供医疗诊断，也不替代医生、心理治疗师或紧急服务。
...
- Chroma 向量数据库
- OpenAI-compatible API / Ollama 本地模型 / 本地微调模型接入配置
- Spring Mail
- EasyExcel
...
管理员接口 `GET /api/admin/model/runtime` 可查看当前模型提供方、聊天模型、嵌入模型和微调适配器配置。该接口不会返回 API Key。
...
高风险流程会写入 `risk_record`，调用 `EmailAlertTool`，并在 `email_alert_log` 记录发送状态。默认 `app.mail.enabled=false`，不会真实发送；配置真实 SMTP 并改为 `true` 后会发送邮件给 `app.mail.alert-receiver`。

真实邮件需要配置两类地址：

- `spring.mail.username`：SMTP 登录账号，通常就是发件邮箱。
- `app.mail.from`：邮件 From 发件人地址，通常和 `spring.mail.username` 相同。
- `app.mail.alert-receiver`：高风险预警收件人地址。

建议把真实邮箱、授权码和收件人写在本地 `backend/src/main/resources/application-local.yml`，该文件已加入 `.gitignore`，不会上传 GitHub：
...
后台接口 `POST /api/admin/email/test` 可用于测试真实邮件链路，发送结果会写入 `email_alert_log`。

## 模型切换

默认使用 OpenAI-compatible API。聊天模型和嵌入模型是两个配置：聊天模型用于意图识别、风险分类和回复生成；嵌入模型用于知识库文档和用户问题向量化，是 RAG 检索必须的。若部署本地微调模型，将 `llm.provider` 切换为 `ollama` 并配置本地模型名即可。
